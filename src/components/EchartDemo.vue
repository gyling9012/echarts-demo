...
import '../../node_modules/echarts/china.js' // 引入中国地图数据
...
export default {
  ...
  methods: {
    ...
    drawCharts() {
      // 数据为城市名 地图库 声明了一个变量 变量名为geoCoordMap
      var geoCoordMap = {
        上海: [121.4648, 31.2891],
        东莞: [113.8953, 22.901],
        东营: [118.7073, 37.5513],
      ...
      // name value键值对 声明了一个变量 变量名为BJData
      var BJData = [
        [
          {
            name: '海南'
      ...
      myChart.on('click', function name(params) {
        console.log(series)
        console.log(params)
        series.forEach(function(item, i) {
          if ('lines' === item.type) {
            if (params.name === item.name) {
              item.effect.show = true
            } else {
      ...
      // 声明了一个变量 变量名 series （为编号；序列号；系列序号）
      var series = [
        {
          type: 'map',
          map: 'china',
          ...
          itemStyle: {
            normal: {
              borderColor: '#ccc',
              shadowColor: 'rgba(200, 280, 250, 0.05)',
              shadowOffsetX: 0,
              shadowOffsetY: 0,
              shadowBlur: 120
            },
        ...
        geo: {
          ...
          itemStyle: {
            normal: {
              ...
              borderWidth: 2,
              borderColor: '#a7e4e6',
              shadowColor: 'rgba(166, 230, 236, 10)',
              shadowOffsetX: 0,
              shadowOffsetY: 0,
              shadowBlur: 120
            },
